...
static inline int newstrcmp_aligned_loop(const op_t *x1,
                                         const op_t *x2,
                                         op_t w1)
{
    op_t w2 = *x2++;
...
/* Unaligned loop: align the first partial of P2, with 0xff for the rest of
   ...
   final comparison.  */
static inline int newstrcmp_unaligned_loop(const op_t *x1,
                                           const op_t *x2,
                                           op_t w1,
                                           uintptr_t ofs)
{
...
int newstrcmp(const char *p1, const char *p2)
{
    ...
    return ofs == 0 ? newstrcmp_aligned_loop(x1, (const op_t *) p2, w1)
                    : newstrcmp_unaligned_loop(x1, (const op_t *) (p2 - ofs),
                                               w1, ofs);
}
